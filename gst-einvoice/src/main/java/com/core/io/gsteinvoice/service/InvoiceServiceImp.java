package com.core.io.gsteinvoice.service;

import com.core.io.gsteinvoice.dto.request.InvoiceRequestDTO;
import com.core.io.gsteinvoice.dto.request.ItemDTO;
import com.core.io.gsteinvoice.dto.request.ValDtls;
...
public class InvoiceServiceImp {
    ...
    public InvoiceResponseDTO generateAndPostInvoice(InvoiceRequestDTO invoice) {
        ...
        try {
            ...
            return InvoiceResponseDTO.builder()
                    .status(root.path("status").asInt())
                    .message(root.path("message").asText())
                    .Irn(root.path("Irn").asText())
                    .uuid(root.path("uuid").asText())
                    .AckDt(root.path("AckDt").asText())
                    .AckNo(root.path("AckNo").asLong())
                    .EwbDt(getNullableText(root, "EwbDt"))
                    .EwbNo(getNullableText(root, "EwbNo"))
                    .Status(root.path("Status").asText())
                    .Remarks(getNullableText(root, "Remarks"))
                    .AckNoStr(root.path("AckNoStr").asText())
                    .EwbValidTill(getNullableText(root, "EwbValidTill"))
                    .SignedQRCode(root.path("SignedQRCode").asText())
                    .SignedInvoice(root.path("SignedInvoice").asText())
                    .SignedQrCodeImgUrl(root.path("SignedQrCodeImgUrl").asText())
                    .InvoicePdfUrl(getNullableText(root, "InvoicePdfUrl"))
                    .DigitallySignedInvoicePdfUrl(getNullableText(root, "DigitallySignedInvoicePdfUrl"))
                    .EWayBillPdfUrl(getNullableText(root, "EWayBillPdfUrl"))
                    .EWayBillQrCodeUrl(getNullableText(root, "EWayBillQrCodeUrl"))
                    .EWayBillPartaSlipPdfUrl(getNullableText(root, "EWayBillPartaSlipPdfUrl"))
                    .IrnStatus(root.path("IrnStatus").asText())
                    .EwbStatus(root.path("EwbStatus").asText())
                    .Irp(root.path("Irp").asText())
                    .IrpPortal(getNullableText(root, "IrpPortal"))
                    .EwbPortal(getNullableText(root, "EwbPortal"))
                    .build();

        } catch (Exception e) {
            log.error("Failed to parse GSTZen response", e);
            throw new RuntimeException("Failed to parse response from GSTZen");
        }
    }
    ...
    private String getNullableText(JsonNode root, String fieldName) {
